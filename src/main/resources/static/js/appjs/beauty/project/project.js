...
function operateFormatter(value, row, index) {
    ...
    var p = "";
    if (s_check_h != "hidden" && row.isUp == '0') {
        p = '<a class="btn btn-warning btn-sm ' + s_check_h + '" href="#" title="发布"  mce_href="#" onclick="pass(\''
            + row.id + '\')"><i class="fa fa-check"></i></a> ';
    }
    var d = "";
    if (s_check_h != "hidden" && row.isUp == '1') {
        d = '<a class="btn btn-warning btn-sm ' + s_check_h + '" href="#" title="下架"  mce_href="#" onclick="deny(\''
...
function isUpFormatter(value, row, index) {
    if (value == '0') {
        return "否";
    }
    if (value == '1') {
...
function photo() {
    ...
    layer.open({
        ...
        area: ['1000px', '620px'],
        content: prefix + '/photo/' + rows[0].id  // iframe的url
    });
}
