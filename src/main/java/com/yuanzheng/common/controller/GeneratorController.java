...
import com.yuanzheng.common.service.GeneratorService;
import com.yuanzheng.common.utils.GenUtils;
...
public class GeneratorController {
    ...
    public void batchCode(HttpServletRequest request, HttpServletResponse response, String tables) throws Exception {
        ...
        byte[] data = generatorService.generatorCode(tableNames);
        response.reset();
        response.setHeader("Content-Disposition", "attachment; filename=\"onezor.zip\"");
        response.addHeader("Content-Length", "" + data.length);
        response.setContentType("application/octet-stream; charset=UTF-8");
